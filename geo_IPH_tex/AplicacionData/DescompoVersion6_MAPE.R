...
modelo = lm(PIB_model$PIB~as.matrix(Zprop))
...
#---------------------------------------------
# Helper: SAR-AR(1) covariance builder
#---------------------------------------------
build_cov <- function(W, rho, phi, sigma2, T) {
  n <- nrow(W)
  A_spatial <- diag(n) - rho * W
  A_inv <- solve(A_spatial)
  
  # Temporal covariance (AR1)
  Sigma_t <- sigma2 / (1 - phi^2) * toeplitz(phi^(0:(T-1)))
  
  # Full covariance (Kronecker)
  B <- kronecker(Sigma_t, A_inv %*% t(A_inv))
  return(list(A_inv = A_inv, B = B))
}

#---------------------------------------------
# Build aggregation & anchoring matrices
#---------------------------------------------
build_Cobs <- function(c_vec, n, T, d) {
  I_d <- diag(d * n)
  I_Td <- diag(T - d)
  C22 <- kronecker(I_Td, t(c_vec))
  Cobs <- rbind(I_d, cbind(matrix(0, 0, 0), C22)) # block binding
  # Actually simpler:
  Cobs <- rbind(diag(d * n), kronecker(diag(T - d), t(c_vec)))
  return(Cobs)
}

#---------------------------------------------
# Log-likelihood function (profile)
#---------------------------------------------
loglik_obs <- function(theta, Ya, Y1d, Z, c_vec, W, d) {
  rho   <- theta[1]
  phi   <- theta[2]
  sigma2 <- exp(theta[3])  # ensure positivity
  n <- nrow(W)
  Tt <- nrow(Z) / n
  res_cov <- build_cov(W, rho, phi, sigma2, Tt)
  A_inv <-kronecker(diag(Tt),res_cov$A_inv)
  B <- res_cov$B
  ...
  C22 <- kronecker(diag(Tt - d), t(c_vec))
  G2 <- rbind(
    cbind(B11, B12 %*% t(C22)),
    cbind(C22 %*% B21, C22 %*% B22 %*% t(C22))
  )
  
  Yobs <- matrix(c(Y1d, Ya[(d+1):Tt]), ncol=1)
  
  # Compute GLS beta
  Cobs <- bdiag(diag(d*n), kronecker(diag(Tt - d), t(c_vec)))
  P <- A_inv %*% Z
  G2inv <- solve(G2)
  M <- t(P) %*% t(Cobs) %*% G2inv %*% Cobs %*% P
  b_hat <- solve(M, t(P) %*% t(Cobs) %*% G2inv %*% Yobs)
  
  mu_obs <- Cobs %*% A_inv %*% Z %*% b_hat
  resid <- Yobs - mu_obs
  
  ll <- -0.5 * (log(det(G2)) + t(resid) %*% G2inv %*% resid)
...
#---------------------------------------------
fit_model <- function(W, Ya, Y1d, Z, c_vec, d) {
  f <- function(par) -loglik_obs(par, Ya, Y1d, Z, c_vec, W, d)
  opt <- optim(par = c(0.1, 0.3, log(1)), fn = f, method = "L-BFGS-B",
               lower = c(-0.999, -0.999,-10),upper = c(0.999, 0.999,2))
...
#---------------------------------------------
# Conditional mean (disaggregation)
#---------------------------------------------
predict_disagg <- function(W, Z, c_vec, theta_hat, d, Y1d, Ya) {
  rho <- theta_hat[1]
  phi <- theta_hat[2]
  ...
  covs <- build_cov(W, rho, phi, sigma2, Tt)
  A_inv <-kronecker(diag(Tt),covs$A_inv)
  B <- covs$B
  ...
  P <- A_inv %*% Z
  M <- t(P) %*% t(Cobs) %*% G2inv %*% Cobs %*% P
  b_hat <- solve(M, t(P) %*% t(Cobs) %*% G2inv %*% c(Y1d, Ya[(d+1):Tt]))
  
  mu <- A_inv %*% Z %*% b_hat
  mu1 <- mu[idx1]
  mu2 <- mu[idx2]
  
  Yobs <- c(Y1d, Ya[(d+1):Tt])
  mu_obs <- Cobs %*% mu
  Y2_pred <- mu2 + G1 %*% G2inv %*% (Yobs - mu_obs)
  VarY2_pred <- B22- G1 %*% G2inv %*% t(G1)
  
  return(list(Y2_pred = Y2_pred, beta_hat = b_hat, 
              theta_hat = theta_hat, VarY2_pred=VarY2_pred))
}
...
Yd = PIB_model$PIB/kk
Yd = Yd[1:(d*n)]
cvec = matrix(1,ncol=1, nrow=n)
Z2 = Z

theta_hat <- fit_model(W=W, Ya=Ya, Y1d=Yd, Z=Z2, c_vec=cvec, d=d)

res <- predict_disagg(W=W, Z=Z2, c_vec=cvec, theta_hat, d=d, Y1d=Yd, Ya=Ya)
...
Ypredict <- cbind(c(Yd, res$Y2_pred[,1]))
VarY2 <- bdiag(0*diag(n*d), res$VarY2_pred)

PIB_model$Ydesa <- (matrix(runif(n),ncol=1, nrow=n*Tt)+Ypredict)*kk
PIB_model$YAncla <- Ypredict*kk
...
sd_YAncla <- sqrt(diag(VarY2+0.001))*kk#/sqrt(n-1)
...
colores <- c(
  ...
  "Ydesa" = "purple",   # Azul fuerte: se ve bien en B/N
  "YAncla" = "blue"   # Verde azulado: también bien distinguible
)

ltys <- c(
  "PIB"    = "solid",   # Negro puro: dato observado
  "Ydesa" = "dashed",   # Azul fuerte: se ve bien en B/N
  "YAncla" = "dotted"   # Verde azulado: también bien distinguible
)


# cols <- c("Low" = "red", "Medium" = "blue", "High" = "green4", "Very High" = "purple")
#ltys <- c("Low" = "solid", "Medium" = "dashed", "High" = "dotted", "Very High" = "dotdash")
...
# 4. Gráfico top 9 (3x3)
# Ejemplo para top9
p_top9 <- ggplot(PIB_top9, aes(x = anio, y = valor / 1e7,
  ...
  theme_minimal(base_size = 10) +
  labs(x = "Year", y = "GDP (hundreds of millions of €)", color = NULL,
       fill = NULL) +
  theme(
...
# Ejemplo para top9
p_bottom8 <- ggplot(PIB_bottom8, aes(x = anio, y = valor / 1e7,
                                     color = variable, linetype=variable)) +
  geom_ribbon(
