...
write_csv2(resultados_final, file="Seleccion.csv")
...
PIB_model$Ydesa <- as.numeric(res$Y_hat)*kk
PIB_model$Ynaive <- as.vector(kk*matrix(Ya, n, TT, byrow=TRUE)/n)
PIB_model$YAncla <- as.numeric(res$Y_hat_H)*kk 


# Datos en formato largo
PIB_long <- PIB_model %>%
  pivot_longer(cols = c(PIB, Ydesa, YAncla, Ynaive),
               names_to = "variable",
               values_to = "valor")


# Vector de desviación estándar
sd_YAncla <- sqrt(diag(res$VarYAncla))*kk/sqrt(n-1)
...
p_top9 <- ggplot(PIB_top9, aes(x = anio, y = valor / 1e7,
  ...
  scale_color_manual(values = colores,
                     labels = c("Observed", "Disaggregated (no anchor)", "Disaggregated (anchor)"),
...
# Vector de desviación estándar
sd_YAncla <- sqrt(diag(res$VarYAncla))*kk/sqrt(2*n)
...
tab = PIB_model %>% 
  ...
  summarise(MAPE=100*round(Metrics::mape(PIB,YAncla),4),
            ...
            MAPENaive=100*round(Metrics::mape(PIB,Ynaive),4)) %>% 
...
sigma2 = 0.2707
phi1 = 0.8961
AA = kronecker(diag(TT), diag(n)-rho*W2)
JJ = matrix(1,n, n)
uno1n = matrix(1, n,1)
FF = diag(n)-rho*W2
Z = ZPCA
C = kronecker(diag(TT), matrix(1,ncol=n, nrow=1))
Ya = Ya
mm = Ya - C%*%solve(AA)%*%Z%*%beta
sigmaU = sigma2/(1-phi1^2)*ar1_cor(TT, phi1)

mm=mm-sum(mm)/TT
mrho = t(uno1n)%*%solve(FF)%*%solve(t(FF))%*%uno1n
Drho = solve(FF)%*%solve(t(FF))%*%uno1n/mrho[1,1]
krho= t(Drho)%*%(diag(n)-JJ/n)%*%Drho
mu = solve(AA)%*%Z%*%beta
mua = C%*%solve(AA)%*%Z%*%beta 
tildemurho = 2*t(mu)%*%kronecker(diag(TT),Drho- uno1n/n)%*%mm
PP= kronecker(diag(TT), JJ/n)

P1 = kronecker(diag(TT), solve(FF)%*%solve(t(FF))%*%JJ/mrho[1,1])
P2 = kronecker(solve(sigmaU), JJ/n)
MM = (diag(n*TT)-P1)%*%solve(AA)%*%Z 
varbeta = (mrho[1,1]/n)*solve(t(solve(AA)%*%Z)%*%P2%*%solve(AA)%*%Z)
varMMbeta = sum(diag(MM%*%varbeta%*%t(MM)))

t(mu)%*%(diag(n*TT)-PP)%*%mu
2*krho*sum(mm^2)
2*t(mu)%*%kronecker(diag(TT),Drho- uno1n/n)%*%mm
varMMbeta
